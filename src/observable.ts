import { Action, EventHandler, Observable, Observer, Operator, Subscription } from "./types";
...
export class LightweightObservable<T> implements Observable<T> {
    ...
    private subscribeToObserver(observer: Partial<Observer<T>>): Subscription {
        ...
        try {
            ...
            const subscriber = new LightweightSubscriber<T>(makeFull(observer), subscription);
