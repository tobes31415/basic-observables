 ...
 */
function operatorBase<TIn, TOut, State = any>(
  nextFn: (observer: Observer<TOut>, value: TIn, state: State) => void,
  initialState?: State,
) {
...
export function create<T>(
  fn: (observer: Observer<T>) => void | (() => void) | (() => Promise<void>),
): Observable<T> {
...
export function map<TIn, TOut>(
  ...
  return operatorBase<TIn, TOut>((observer, value) =>
...
export function filter<T>(filterFn: (value: T) => boolean): Operator<T, T> {
  return operatorBase<T, T>((observer, value) => {
    if (filterFn(value)) {
      observer.next(value);
    }
  });
 ...
 */
export function scan<TIn, TOut>(
  scanFn: (acc: TOut | undefined, value: TIn) => TOut,
  initialValue?: TOut,
): Operator<TIn, TOut> {
  return operatorBase<TIn, TOut, { currentValue: TOut | undefined }>(
...
export function take<T>(take: number): Operator<T, T> {
  ...
  return operatorBase<T, T, { count: number }>(
    (observer, value, state) => {
      observer.next(value);
      state.count++;
      if (state.count >= take) {
